...
def plot_shift(df_raw, df_master, units):
    ...
    if df_master.domain == "freq":
        ...
        for _i, (group, df_set) in enumerate(gb_master):
            ...
            (line2,) = ax2.loglog(
                df_set["f"], df_set[loss], ls="", marker=".", color=colors2[int(group)]
            )
            lax1.append(line1)
            lax2.append(line2)
        for i, (group, df_set) in enumerate(gb_raw):
            if i in np.linspace(0, gb_raw.ngroups - 1, num=5, dtype=int):
                ...
                ax1.loglog(
                    df_set["f_set"], df_set[stor], ls="", marker=".", color=colors1[int(group)]
                )
                ax2.loglog(
                    df_set["f_set"], df_set[loss], ls="", marker=".", color=colors2[int(group)]
                )
        ...
        for i, (group, df_set) in enumerate(gb_raw):
            if i in np.linspace(0, gb_raw.ngroups - 1, num=5, dtype=int):
                ...
                ax1.loglog(
                    df_set["t_set"], df_set[relax], ls="", marker=".", color=colors1[int(group)]
                )
        ax = (ax1, lax1)
...
def plot_shift_debug(dshift):
    ...
    for T, shift in sorted(dshift.items()):
        ...
        print(
            "ref-drop-first: {} | perr: {} | perr_rem : {}".format(
        ...
        print(
            "shift-drop-first: {} | perr: {} | perr_rem : {}".format(
