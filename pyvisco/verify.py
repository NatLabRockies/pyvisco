...
def plot_fit_ANSYS(df_master, df_GMaxw, df_GMaxw_ANSYS, units):
    ...
    if df_master.domain == "freq":
        ...
        df_master.plot(
            ...
            markersize=3,
            alpha=0.5,
            label=[f"{modul}'(Master curve)", f"{modul}''(Master curve)"],
        )
        df_GMaxw.plot(
            x="f",
            y=[stor, loss],
            ...
            ls="-",
            lw=2,
            label=[f"{modul}'(Python)", f"{modul}''(Python)"],
        )
        df_GMaxw_ANSYS.plot(
            x="f",
            y=[stor, loss],
            ...
            ls="-",
            lw=2,
            label=[f"{modul}'(ANSYS)", f"{modul}''(ANSYS)"],
        )
        ax1.set_xlabel("Frequency ({})".format(units["f"]))
        ax1.set_ylabel(f"Storage and loss modulus ({units[stor]})")
        ax1.legend()
        fig.show()
        return fig
