...
def conventions(modul):
    ...
    conv = {
        ...
        "tan_del": ["-", ""],
        "log_aT": ["-", ""],
        f"{m}_relax": ["Pa", "kPa", "MPa", "GPa"],
        f"{m}_stor": ["Pa", "kPa", "MPa", "GPa"],
        f"{m}_loss": ["Pa", "kPa", "MPa", "GPa"],
        f"{m}_comp": ["Pa", "kPa", "MPa", "GPa"],
        f"{m}_0": ["Pa", "kPa", "MPa", "GPa"],
        f"{m}_inf": ["Pa", "kPa", "MPa", "GPa"],
        f"{m}_i": ["Pa", "kPa", "MPa", "GPa"],
        f"{m}_relax_filt": ["Pa", "kPa", "MPa", "GPa"],
        f"{m}_stor_filt": ["Pa", "kPa", "MPa", "GPa"],
        f"{m}_loss_filt": ["Pa", "kPa", "MPa", "GPa"],
        "Set": ["-", ""],
        "RefT": ["C", "°C"],
        "C1": ["-"],
        "C2": ["C", "°C"],
...
def Eplexor_raw(data, modul):
    ...
    df.rename(
        columns={
            "f": "f_set",
            f"{modul}'": f"{modul}_stor",
            f"{modul}''": f"{modul}_loss",
            f"|{modul}*|": f"{modul}_comp",
            "tan delta": "tan_del",
        },
        inplace=True,
        errors="raise",
    )
    df_raw = df[["f_set", f"{modul}_stor", f"{modul}_loss", f"{modul}_comp", "tan_del", "T"]].copy()
...
def user_raw(data, domain, modul):
    ...
    if domain == "freq":
        ...
        df_raw.rename(
            columns={"t": "t_set", f"{modul}_relax": f"{modul}_relax", "T": "T", "Set": "Set"},
            inplace=True,
            errors="raise",
        )
...
def Eplexor_master(data, modul):
    ...
    df_master = df_master_raw[
        ["f", f"{modul}_stor", f"{modul}_loss", f"{modul}_comp", "tan_del"]
    ].copy()
...
def user_master(data, domain, RefT, modul):
    ...
    if domain == "freq":
        df_master.rename(
            columns={"f": "f", f"{modul}_stor": f"{modul}_stor", f"{modul}_loss": f"{modul}_loss"},
            inplace=True,
            errors="raise",
        )
        df_master["omega"] = 2 * np.pi * df_master["f"]
        ...
        df_master.rename(
            columns={"t": "t", f"{modul}_relax": f"{modul}_relax"}, inplace=True, errors="raise"
        )
...
def user_shift(data_shift):
    ...
    df_aT.rename(columns={"T": "T", "log_aT": "log_aT"}, inplace=True, errors="raise")
...
def get_units(units, modul, domain, Eplexor=False):
    ...
    if domain == "freq":
        if Eplexor:
            pascal = units[f"{modul}'"]
        else:
            pascal = units[f"{modul}_stor"]
    elif domain == "time":
        pascal = units[f"{modul}_relax"]
...
def check_units(units, modul="E"):
    ...
    for key in units.keys():
        ...
        if key == f"{m}_stor":
            if units[f"{m}_stor"] != units[f"{m}_loss"]:
                msg = (
